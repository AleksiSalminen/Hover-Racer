...
export default class ShipControls {
	...
	constructor(ctx) {
		...
		if (ctx.controlType == 1 && bkcore.controllers.TouchController.isCompatible()) {
			...
			let leapInfo = this.leapInfo = document.getElementById('leapinfo');
			isServerConnected = false;
			let lb = this.leapBridge = {
				isConnected: true,
				hasHands: false,
				palmNormal: [0, 0, 0]
			};
			...
			let lc = this.leapController = new Leap.Controller({ enableGestures: false });
	...
	update(dt) {
		...
		if (this.mesh != null) {
			...
			let gradientDelta = (this.gradientTarget - (yawLeap + this.gradient)) * this.gradientLerp;
			...
			// Tilting (Idem)
			let tiltDelta = (this.tiltTarget - this.tilt) * this.tiltLerp;
			...
			// Rolling (Idem)
			let rollDelta = (rollAmount - this.roll) * this.rollLerp;
	...
	teleport(pos, quat) {
		...
		if (this.mesh != null) {
			...
			// Gradient (Mesh only, no dummy physics impact)
			let gradientDelta = (this.gradientTarget - this.gradient) * this.gradientLerp;
			if (Math.abs(gradientDelta) > this.epsilon) this.gradient += gradientDelta;
			if (Math.abs(this.gradient) > this.epsilon) {
				this.gradientAxis.set(1, 0, 0);
				this.mesh.matrix.rotateByAxis(this.gradientAxis, this.gradient);
	...
	boosterCheck(dt) {
		...
		let z = Math.round(this.collisionMap.pixels.height / 2 + this.dummy.position.z * this.collisionPixelRatio);
	...
	collisionCheck(dt) {
		...
		if (collision.r < 255) {
			...
			let lPos = this.repulsionVLeft.addSelf(pos);
			let rPos = this.repulsionVRight.addSelf(pos);
			let lCol = this.collisionMap.getPixel(Math.round(lPos.x), Math.round(lPos.z)).r;
			let rCol = this.collisionMap.getPixel(Math.round(rPos.x), Math.round(rPos.z)).r;
			...
			if (rCol > lCol) {// Repulse right
				this.repulsionForce.x += -this.repulsionAmount;
				this.collision.left = true;
			}
			else if (rCol < lCol) {// Repulse left
				this.repulsionForce.x += this.repulsionAmount;
	...
	heightCheck(dt) {
		...
		let height = this.heightMap.getPixelFBilinear(x, z) / this.heightScale + this.heightBias;
	
		let color = this.heightMap.getPixel(x, z);
	
		if (height < 16777) {
			let delta = (height - this.dummy.position.y);
